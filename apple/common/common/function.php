 ...
 * 生成数据返回值
 */
function APPReturn($msg,$status = -1,$data = []){
 ...
 */
function APPCheckFilterWords($srcword,$filterWords){
	$flag = true;
	if($filterWords!=""){
 ...
 */
function APPSendSMS($phoneNumer,$content){
	$url = 'http://utf8.sms.webchinese.cn/?Uid='.APPLEConf("CONF.smsKey").'&Key='.APPLEConf("CONF.smsPass").'&smsMob='.$phoneNumer.'&smsText='.$content;
	$ch=curl_init($url);
 ...
 * 获取指定的全局配置
 */
function APPLEConf($key,$v = ''){
	if(is_null($v)){
		if(array_key_exists('APPLECONF',$GLOBALS) && array_key_exists($key,$GLOBALS['APPLECONF'])){
		    unset($GLOBALS['APPLECONF'][$key]);
		}
	}else if($v === ''){
		if(array_key_exists('APPLECONF',$GLOBALS)){
			$conf = $GLOBALS['APPLECONF'];
			$ks = explode(".",$key);
		...
		return $GLOBALS['APPLECONF'][$key] = $v;
	}
	return null;
}

//php获取中文字符拼音首字母
function APPGetFirstCharter($str){
	if(empty($str)){
		return '';
	}
	$fchar=ord($str{0});
 ...
 * @param int 0-用户  1-商家
 */
function APPLoginTarget($target = 0){
	$WST_USER = session('APP_USER');
	$WST_USER['loginTarget'] = $target;
	session('APP_USER',$WST_USER);
}
/**
 * 邮件发送函数
 ...
 * @return array
 */
function APPSendMail($to, $subject, $content) {
	$mail = new \phpmailer\phpmailer();
    // 装配邮件服务器
    $mail->IsSMTP();
    $mail->SMTPDebug = 0;
    $mail->Host = APPLECONF("CONF.mailSmtp");
    $mail->SMTPAuth = APPLECONF("CONF.mailAuth");
    $mail->Username = APPLECONF("CONF.mailUserName");
    $mail->Password = APPLECONF("CONF.mailPassword");
    $mail->CharSet = 'utf-8';
    // 装配邮件头信息
    $mail->From = APPLECONF("CONF.mailAddress");
    $mail->AddAddress($to);
    $mail->FromName = APPLECONF("CONF.mailSendTitle");
    $mail->IsHTML(true);
    // 装配邮件正文信息
	...
	if(!$rs){
        ...
        if(!empty($styles)){
	        foreach ($styles as $key => $v) {
		        $rs['app'.$v['styleSys'].'Style'] = $v['stylePath'];
		        $rs['app'.$v['styleSys'].'StyleId'] = $v['id'];
	        }
        }
		//获取上传文件目录配置
		...
		$rs['wstUploads'] = $data;
		if(APPLECONF('CONF.mallLicense')=='')$rs['mallSlogan'] = $rs['mallSlogan']."  ".base64_decode('UG93ZXJlZCBCeSBXU1RNYXJ0');
		cache('app_CONF',$rs,31536000);
	}
	return $rs;
} 
 ...
 * @param $key 要检测的内容
 */
function APPCheckLoginKey($val,$userId = 0){
    if($val=='')return APPReturn("登录账号不能为空");
    if(!APPCheckFilterWords($val,APPLECONF("CONF.registerLimitWords"))){
    	return APPReturn("登录账号包含非法字符");
    }
    ...
    $rs = $dbo->count();
    if($rs==0){
    	return APPReturn("该登录账号可用",1);
    }
    return APPReturn("对不起，登录账号已存在");
}

/**
 * 生成随机数账号
 */
function APPRandomLoginName($loginName){
	$chars = array("a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z");
    //简单的派字母
    foreach ($chars as $key =>$c){
    	$crs = APPCheckLoginKey($loginName."_".$c);
    	if($crs['status']==1)return $loginName."_".$c;
    }
    //随机派三位数值
    for($i=0;$i<1000;$i++){
    	$crs = $this->APPCheckLoginKey($loginName."_".$i);
 ...
 */
function APPStrReplace($str,$repStr,$start,$splilt = ''){
 ...
 * 获取指定商品分类的子分类列表
 */
function APPGoodsCats($parentId = 0,$isFloor = -1){
 ...
 */
function APPUploadPic($fromType=0){
	$fileKey = key($_FILES);
	$dir = Input('param.dir');
	if($dir=='')return json_encode(['msg'=>'没有指定文件目录！','status'=>-1]);
	$dirs = APPLECONF("CONF.wstUploads");
    ...
    if($info){
    	...
    	//图片记录
    	APPRecordImages($imageSrc, (int)$fromType);
    	//打开原图
    	...
    	if($isThumb==1){
    		...
    		//是否需要生成移动版的缩略图
    		$suffix = APPLECONF("CONF.wstMobileImgSuffix");
    		if(!empty($suffix)){
    			$image = \image\Image::open($imageSrc);
    	...
    	if($isWatermark==1 && (int)APPLECONF('CONF.watermarkPosition')!==0){
	    	//取出水印配置
	    	$wmWord = APPLECONF('CONF.watermarkWord');//文字
	    	$wmFile = trim(APPLECONF('CONF.watermarkFile'),'/');//水印文件
	    	$wmPosition = (int)APPLECONF('CONF.watermarkPosition');//水印位置
	    	$wmSize = ((int)APPLECONF('CONF.watermarkSize')!=0)?APPLECONF('CONF.watermarkSize'):'20';//大小
	    	$wmColor = (APPLECONF('CONF.watermarkColor')!='')?APPLECONF('CONF.watermarkColor'):'#000000';//颜色必须是16进制的
	    	$wmOpacity = ((int)APPLECONF('CONF.watermarkOpacity')!=0)?APPLECONF('CONF.watermarkOpacity'):'100';//水印透明度
	    	//是否有自定义字体文件
	    	$customTtf = $_SERVER['DOCUMENT_ROOT'].APPLECONF('CONF.watermarkTtf');
...
* 
*/
function APPRecordImages($imgPath, $fromType){
	$data = [];
...
*/
function APPUseImages($fromType, $dataId, $imgPath, $fromTable='', $imgFieldName=''){
...
function WSTImg($imgurl,$imgType = 1){
	$m = APPLECONF('CONF.wstMobileImgSuffix');
...
* 编辑器上传图片
*/
function APPEditUpload($fromType){
    //PHP上传失败
...
*/
function APPSendMsg($to,$content,$msgJson=[],$msgType = 1){
	$message = [];
 ...
 * 获取分类的佣金
 */
function APPGoodsCommissionRate($goodsCatId){
...
function APPFormatIn($split,$str){
